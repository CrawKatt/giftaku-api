...
fn random_file(path: &String) -> std::io::Result<String> {
    ...
    Err(std::io::Error::new(
        ...
        "Error in random_file function: No files in directory",
    ))
}
...
pub async fn send_result(action: &str) -> Result<Json<ResponseData>, std::io::Error> {
    ...
    let query_result: Vec<SaveData> = DB
        .query(sql_query)
    ...
    if query_result.is_empty() {
        return Err(std::io::Error::new(
            std::io::ErrorKind::Other,
            "Error in GET: No results in query",
        ));
    }
    ...
    let response_data = ResponseData::new(
        anime_name,
        format!("https://{}/api/{action}/{file_name}", *URL_HOST),
    );
...
pub async fn get_gif(action: &str, file_name: &str) -> Option<NamedFile> {
    NamedFile::open(Path::new("./upload/").join(action).join(file_name))
        .await
        .ok()
}
